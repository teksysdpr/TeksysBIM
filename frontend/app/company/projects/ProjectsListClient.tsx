      ...
      <div className="w-full max-w-md rounded-2xl border border-[#3f2d1a] bg-[#120c07] p-6 shadow-[0_24px_64px_rgba(0,0,0,0.6)]">
        ...
        <div className="mb-5 flex items-center justify-between">
          <div>
            ...
            <h2 className="mt-0.5 text-lg font-black text-[#f8e8cf]">Create BIM Project</h2>
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="mb-1.5 block text-xs font-bold text-[#c4a46e]">Project Code <span className="text-[#f87171]">*</span></label>
          ...
          <div>
            <label className="mb-1.5 block text-xs font-bold text-[#c4a46e]">Project Name <span className="text-[#f87171]">*</span></label>
            <input value={name} onChange={(e) => setName(e.target.value)}
              placeholder="e.g. Prestige Heights — CAD2BIM" className={FIELD} required />
          </div>

          <div>
            <label className="mb-1.5 block text-xs font-bold text-[#c4a46e]">Client Name <span className="text-[#f87171]">*</span></label>
...
export default function ProjectsListClient() {
  ...
  // Open modal when ?action=new is in URL
  useEffect(() => {
  ...
  function handleCreated(project: ProjectListItem) {
    setShowModal(false);
    router.replace("/company/projects");
    // Optimistically add to list then reload for full data
  ...
  function openModal() {
    setShowModal(true);
    router.replace("/company/projects?action=new", { scroll: false });
  }

  function closeModal() {
    setShowModal(false);
    router.replace("/company/projects", { scroll: false });
  }
  ...
  return (
    <BimPageShell>
      {showModal && (
        <CreateProjectModal onClose={closeModal} onCreated={handleCreated} />
      )}

      {/* ── Page header ── */}
      <BimPageHeader
        eyebrow="TeksysBIM · Projects"
        title="Projects"
        subtitle={subtitle}
        action={
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          {!search && statusFilter === "ALL" && (
            <>
              <p className="mt-1 text-sm text-[#7a5e3e]">Create your first BIM project to get started.</p>
